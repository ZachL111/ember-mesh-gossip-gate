...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-ember-mesh-gossip-detail.ps1

sqlite3 :memory: ".read tests/domain_review.sql"

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-ember-mesh-gossip-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-ember-mesh-gossip-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-ember_mesh_gossip_gate-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
